...
import requests
import os
from utils import generate_device_token, respond_to_challenge, inputs_to_set, format_response, init_response
import robin_stocks.urls as urls
import getpass
import pickle
from config import CONFIG

class robin_stock_client:
...
#            raise
    ...
    def login(self, username=None, password=None, expiresIn=86400, scope='internal', by_sms=True, store_session=True):
        ...
        if username is None or password is None:
            username = CONFIG['flask']['RH_USER']
            password = CONFIG['flask']['RH_PW']
        ...
        if os.path.isfile(pickle_path):
            ...
            if store_session:
                try:
                    ...
                    self._update_session('Authorization', None)
            else:
        ...
        data = self._request_post(url, payload)
        # Handle case where mfa or challenge is required.
        if 'mfa_required' in data:
            mfa_token = input("Please type in the MFA code: ")
            payload['mfa_code'] = mfa_token
            res = self._request_post(url, payload, jsonify_data=False)
        ...
        if 'access_token' in data:
            ...
            self._update_session('Authorization', token)
            self._set_login_state(True)
    ...
    def _request_post(self, url, payload=None, timeout=16, json=False, jsonify_data=True):
        ...
        try:
            if json:
                self._update_session('Content-Type', 'application/json')
    ...
    def _request_get(self, url, dataType='regular', payload=None, jsonify_data=True):
    ...
    def _update_session(self, key, value):
